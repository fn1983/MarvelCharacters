...
protocol NetworkService {
    typealias CompletionHandler = (Result<Data?, MarvelCharactersError.NetworkError>) -> Void
...
final class DefaultNetworkService {
    ...
    private func request(request: URLRequest, completion: @escaping CompletionHandler) -> NetworkCancellable {
        let sessionDataTask = self.sessionManager.request(request) { data, response, requestErrorr in
            if let requestError = requestErrorr {
                var error: MarvelCharactersError.NetworkError
    ...
    private func resolve(error: Error) -> MarvelCharactersError.NetworkError {
...
extension MarvelCharactersError.NetworkError {
